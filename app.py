...
# ==================================================
# ZONA HORARIA (Ecuador UTC-5, sin pytz)
# ==================================================
...
def is_match_editable(match, usuario_id=None):
    ...
    if not match.fecha_hora:
        return True
    if usuario_id:
        intentos = PronosticoPartido.query.filter_by(usuario_id=usuario_id, partido_id=match.id).count()
        if intentos >= 2:
            return False
...
def api_guardar_pronostico_partido():
    ...
    intentos_actuales = PronosticoPartido.query.filter_by(usuario_id=session['user_id'], partido_id=partido_id).count()
    ...
    db.session.commit()
    log_auditoria('CREATE', 'pronostico', pronostico.id, f"Pronóstico tipo={tipo} creado (intento {nuevo_intento}/2)")
...
def api_certificado_campeon():
    ...
    try:
        ...
        c.save()
        buffer.seek(0)
        return send_file(buffer, as_attachment=True, download_name=f"certificado_campeon_{usuario.username}_{datetime.now().strftime('%Y%m%d')}.pdf", mimetype='application/pdf')
    except ImportError:
...
def api_subir_avatar():
    ...
    if file and allowed_file(file.filename):
        ...
        filename = secure_filename(f"user_{session['user_id']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{extension}")
...
def api_mis_pronosticos_pdf():
    ...
    try:
        ...
        def nueva_pagina():
            ...
            c.setFillColor(colors.white)
            c.drawCentredString(width / 2, height - 20, "Polla Mundialista 2026 — continuación")
            c.setFillColor(colors.black)
            return height - 60
        c.setFillColor(colors.HexColor('#003580'))
        c.rect(0, height - 70, width, 70, fill=True, stroke=False)
        ...
        y -= 18
        c.drawString(50, y, f"Puntos Totales: {puntos_obj.puntos_totales if puntos_obj else 0}  |  Exactos: {puntos_obj.resultados_exactos if puntos_obj else 0}  |  Emitido: {datetime.now().strftime('%d/%m/%Y %H:%M')}")
        y -= 25
        ...
        for idx, (pron, partido) in enumerate(pronosticos):
            ...
            visit_nombre = partido.visitante.nombre if partido.visitante else '?'
            partido_str = f"{local_nombre} vs {visit_nombre}"
            tipo = pron.tipo_pronostico or 'marcador'
            if tipo == 'ganador':
                if pron.ganador == 'local':
                    pron_str = f"Gana: {local_nombre}"
                elif pron.ganador == 'visitante':
                    pron_str = f"Gana: {visit_nombre}"
                else:
                    pron_str = "Empate"
            elif tipo == 'marcador':
                pron_str = f"{pron.goles_local}-{pron.goles_visitante}"
            elif tipo == 'penales':
                pron_str = f"{pron.goles_local}-{pron.goles_visitante} (pen: {pron.penales_local}-{pron.penales_visitante})"
            else:
                pron_str = "—"
            if partido.goles_local is not None:
                if partido.penales_local is not None:
                    res_str = f"{partido.goles_local}-{partido.goles_visitante} (pen: {partido.penales_local}-{partido.penales_visitante})"
                else:
        ...
        c.drawCentredString(width / 2, 11, '"El fútbol es pasión, los pronósticos son emoción. ¡A jugar!" — Polla Mundialista 2026')
        c.save()
        buffer.seek(0)
        return send_file(buffer, as_attachment=True, download_name=f"pronosticos_{usuario.username}_{datetime.now().strftime('%Y%m%d')}.pdf", mimetype='application/pdf')
    except ImportError:
...
def api_admin_estadisticas():
    ...
    top_usuarios = db.session.query(Usuario, PuntoTotal).join(PuntoTotal, Usuario.id == PuntoTotal.usuario_id).order_by(PuntoTotal.puntos_totales.desc()).limit(5).all()
    return jsonify({
        'total_usuarios': total_usuarios,
        'total_partidos': total_partidos,
        'partidos_finalizados': partidos_finalizados,
        'total_pronosticos': total_pronosticos,
        'top_usuarios': [{'username': u.username, 'nombre_completo': u.nombre_completo, 'puntos_totales': p.puntos_totales} for u, p in top_usuarios]
    })
    ...
    try:
        ...
        if 'avatar' in request.files:
            ...
            if file and file.filename and allowed_file(file.filename):
                extension = file.filename.rsplit('.', 1)[1].lower()
                filename = secure_filename(f"user_{nuevo_usuario.id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{extension}")
                filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
                file.save(filepath)
                nuevo_usuario.foto_perfil = f'/static/avatars/{filename}'
                db.session.commit()
        return jsonify({'success': True, 'user_id': nuevo_usuario.id, 'message': 'Usuario creado exitosamente'})
    except Exception as e:
        db.session.rollback()
...
def admin_crear_partido():
    ...
    db.session.commit()
    log_auditoria('CREATE', 'partido', nuevo_partido.id, f'Partido creado: {local.nombre} vs {visitante.nombre}')
...
def admin_backup():
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        tablas = ['usuarios_polla', 'selecciones', 'partidos', 'pronosticos_partidos', 'pronosticos_especiales', 'puntos_totales']
        modelos = {
            'usuarios_polla': Usuario, 'selecciones': Seleccion, 'partidos': Partido,
            'pronosticos_partidos': PronosticoPartido, 'pronosticos_especiales': PronosticoEspecial, 'puntos_totales': PuntoTotal
        }
        for tabla in tablas:
    ...
    return send_file(zip_buffer, as_attachment=True, download_name=f'backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}.zip', mimetype='application/zip')
...
def admin_restore():
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(zip_data), 'r') as zf:
            json_content = zf.read('backup_polla.json').decode('utf-8')
        data = json.loads(json_content)
        # Limpiar
        db.session.query(PronosticoPartido).delete()
        db.session.query(PronosticoEspecial).delete()
        db.session.query(PuntoTotal).delete()
        db.session.query(Partido).delete()
        db.session.query(Usuario).delete()
        db.session.query(Seleccion).delete()
        for s in data['selecciones']:
            db.session.add(Seleccion(id=s['id'], nombre=s['nombre'], bandera_url=s['bandera_url'], grupo=s['grupo']))
        for u in data['usuarios']:
            db.session.add(Usuario(id=u['id'], username=u['username'], email=u['email'], password_hash='', nombre_completo=u['nombre_completo'], es_admin=u['es_admin'], activo=u['activo'], foto_perfil=u['foto_perfil'], campeon_id=u.get('campeon_id')))
        for p in data['partidos']:
            db.session.add(Partido(id=p['id'], seleccion_local_id=p['local_id'], seleccion_visitante_id=p['visitante_id'], fecha_hora=datetime.fromisoformat(p['fecha_hora']), fase=p['fase'], grupo=p['grupo'], goles_local=p['goles_local'], goles_visitante=p['goles_visitante'], penales_local=p['penales_local'], penales_visitante=p['penales_visitante'], estado=p['estado'], bloqueado_manual=p['bloqueado_manual']))
        for pr in data['pronosticos']:
            db.session.add(PronosticoPartido(id=pr['id'], usuario_id=pr['usuario_id'], partido_id=pr['partido_id'], tipo_pronostico=pr['tipo'], ganador=pr['ganador'], goles_local=pr['goles_local'], goles_visitante=pr['goles_visitante'], penales_local=pr['penales_local'], penales_visitante=pr['penales_visitante'], intento_numero=pr['intento_numero']))
        for pu in data['puntos']:
            db.session.add(PuntoTotal(usuario_id=pu['usuario_id'], puntos_totales=pu['puntos_totales'], puntos_fase_grupos=pu['puntos_fase_grupos'], puntos_eliminatorias=pu['puntos_eliminatorias'], puntos_especiales=pu['puntos_especiales'], resultados_exactos=pu['resultados_exactos']))
        db.session.commit()
        return jsonify({'success': True})
    except Exception as e:
        db.session.rollback()
...
def init_db():
    with app.app_context():
        ...
        if Partido.query.count() == 0:
            ...
            elim_raw = [
                ("2026-06-28", "14:00", "R32"), ("2026-06-29", "12:00", "R32"), ("2026-06-29", "15:30", "R32"),
                ("2026-06-29", "20:00", "R32"), ("2026-06-30", "12:00", "R32"), ("2026-06-30", "16:00", "R32"),
                ("2026-06-30", "20:00", "R32"), ("2026-07-01", "11:00", "R32"), ("2026-07-01", "15:00", "R32"),
                ("2026-07-01", "19:00", "R32"), ("2026-07-02", "14:00", "R32"), ("2026-07-02", "18:00", "R32"),
                ("2026-07-02", "22:00", "R32"), ("2026-07-03", "13:00", "R32"), ("2026-07-03", "17:00", "R32"),
                ("2026-07-03", "20:30", "R32"), ("2026-07-04", "12:00", "R16"), ("2026-07-04", "16:00", "R16"),
                ("2026-07-05", "15:00", "R16"), ("2026-07-05", "19:00", "R16"), ("2026-07-06", "14:00", "R16"),
                ("2026-07-06", "19:00", "R16"), ("2026-07-07", "11:00", "R16"), ("2026-07-07", "15:00", "R16"),
                ("2026-07-09", "15:00", "QF"), ("2026-07-10", "14:00", "QF"), ("2026-07-11", "16:00", "QF"),
                ("2026-07-11", "20:00", "QF"), ("2026-07-14", "14:00", "SF"), ("2026-07-15", "14:00", "SF"),
                ("2026-07-18", "16:00", "3P"), ("2026-07-19", "14:00", "FINAL"),
            ]
